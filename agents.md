...
The current authorization is **G-NEWS-03: the corrected lo-fi MP3, six-feature town and news completion record requested on 19 September 2026**. Boey explicitly requested the new soundtrack, the missing building with the LLM simulator in an office, arrangement of all six planned features and documentation marking Cyber News Central completed. This accepts G-NEWS/API-01 and G-NEWS-02. The five scored games remain generic previews; their gameplay and separately managed NPC work are outside scope. Preserve prior Times work and unrelated changes. Historical UI-06 hover-card Task 3 remains skipped. Future changes require their own approved scope; do not recreate the completed Guardian service.
...
Follow [feature organization](TASKS.md#feature-organization) for every new feature. Group feature-owned components, composables, styles, assets and request wrappers under its frontend module, and routes/controllers/services/validators/provider adapters/feature-owned models under its server module. Use `client/src/features/<feature-id>/` for new non-game features unless an established module path applies; retain the news paths specified below. Group tests by feature within the existing test-discovery patterns. Plans and handovers must map the feature's client/server flow and exact shared integration edits. Do not relocate unrelated existing modules or duplicate shared services merely to satisfy folder grouping.
...
The [shared blueprint](TASKS.md#shared-implementation-blueprint) and five scored-game schemas/endpoints are proposed; they do not exist merely because documentation describes them. The G-NEWS reading service and Times/OWASP extension are completed and accepted by Boey on 19 September 2026. Keep prototypes labeled unsaved until the authorized shared persistence/auth foundation exists.

**News identity:** G-NEWS / Cyber News Central is “The daily briefing”: catalog ID `threat-briefing`, route `/games/threat-briefing`, `newsstand` at `{ x: 144, y: 480 }`. Its single catalog entry opens The Sharlock Times over the mounted town through a nested route, using `client/src/games/threat-briefing/` and `server/src/modules/news/`. Registered-key verification allowed `isPreview: false`; the other five generic activity previews now match the scored-game IDs and placements listed in DESIGN.md, including the social-engineering office. These previews are existing work; any later game implementation needs explicit continuation approval and must reuse its entry instead of adding a second building. Boey explicitly authorized this continuation and the town/router/About integrations, superseding the old hut/preview baseline. Do not create a second news entry, building, card registry or route identity. Future kickoff agents must treat this as existing work and recheck new overlaps. Browser-local headline Q&A remains deferred.
...
Retain muted/paused-by-default `/audio/sharlock-bgm.mp3`, gesture-driven play, rejected-play retry, no persistent unmuted preference and cleanup on departure/disposal. The explicitly approved newspaper audio is separate: attempt its supplied lo-fi MP3 loop (`/audio/Lo-fi_8-bit_coffee_s_1-1789752418868.mp3`, filename normalized without changing its audio bytes) when the Times opens, provide mute and blocked-play retry, stop on close, and suspend/resume the existing town player according to its prior user choice. Keep this media state feature-local and nonpersistent. Keep the mobile offcanvas focus trap, dismissal restoration and resize cleanup.
