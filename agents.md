...
The current authorization is **G-NEWS / API-01: implement the confirmed Gazette plan in this chat**. Boey explicitly replaced the handover-only instruction with “let's implement it directly in this chat” and authorized the complete confirmed scope. Continue the known `threat-briefing` preview, replace its hut sprite with a newsstand, integrate the routed newspaper and Guardian service/cache, make the town the entry page and consolidate landing/About content. The TASKS.md active record declares exact module/shared integration boundaries. Do not repeat settled questions or apply the standard collision stop to the expressly approved baseline; recheck new overlaps. Preserve unrelated changes. Task 3 remains skipped; Tasks 4–5 are accepted.
...
The [shared blueprint](TASKS.md#shared-implementation-blueprint) and five scored-game schemas/endpoints are proposed; they do not exist merely because documentation describes them. The G-NEWS reading service is implemented and awaiting review. Keep prototypes labeled unsaved until the authorized shared persistence/auth foundation exists.

**News identity:** G-NEWS / Cyber News Central is “The daily briefing”: catalog ID `threat-briefing`, route `/games/threat-briefing`, `newsstand` at `{ x: 144, y: 480 }`. Its single catalog entry opens The Sharlock Gazette over the mounted town through a nested route, using `client/src/games/threat-briefing/` and `server/src/modules/news/`. Registered-key verification allowed `isPreview: false`; the other generic activity previews remain unchanged. Boey explicitly authorized this continuation and the town/router/About integrations, superseding the old hut/preview baseline. Do not create a second news entry, building, card registry or route identity. Future kickoff agents must treat this as existing work and recheck new overlaps. Browser-local headline Q&A remains deferred.
